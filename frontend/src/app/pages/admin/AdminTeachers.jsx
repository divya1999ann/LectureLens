...
import { UserPlus, Search, Mail, Users, BookOpen, AlertCircle, RefreshCw } from 'lucide-react';
...
const AdminTeachers = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const teacherList = usersData.results ?? usersData;
            const courseList = coursesData.results ?? coursesData;

            // Group courses by teacher email (what the list serializer exposes)
            const map = {};
            teacherList.forEach(t => { map[t.id] = []; });
            courseList.forEach(course => {
                const teacher = teacherList.find(t => t.email === course.teacher_email);
                if (teacher) map[teacher.id] = [...(map[teacher.id] || []), course];
            });
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
                                    ...
                                    <div className="mt-3">
                                        {subjects.length > 0 ? (
                                            <div className="flex flex-wrap gap-1.5">
                                                {subjects.map(s => (
                                                    <span key={s.id} className="inline-flex items-center gap-1 text-[11px] bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded-md font-medium">
            ...
            <AddTeacherModal open={modalOpen} onOpenChange={setModalOpen} onAdded={fetchData} />
        </div>
    );
};
